...
export default function AddLearnerDialog({
  ...
  const sanitizeUsername = (email: string) => {
    ...
    return `${base || "learner"}${timestamp}`.toLowerCase();
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!password) {
      toast({
        ...
        description: "Generate or enter a password before creating the learner.",
      });
      return;
    }
      ...
      if (formData.organization) {
        try {
          ...
          toast({
            ...
            description:
              "Learner was created, but the organization could not be persisted. You can update it later.",
          });
        }
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader className="flex flex-row items-center justify-between space-y-0">
          <DialogTitle className="text-xl font-bold">Add Learner</DialogTitle>
        </DialogHeader>
