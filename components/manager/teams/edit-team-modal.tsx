...
import { useGetTeamDetailsQuery, useUpdateTeamMutation } from "@/lib/store/api/teamApi";
...
export default function EditTeamModal({ open, onOpenChange, teamId }: EditTeamModalProps) {
  ...
  const { data: teamData, isLoading: isLoadingTeam } = useGetTeamDetailsQuery(teamId!, {
    skip: !teamId,
  });

  const [updateTeam, { isLoading: isUpdating }] = useUpdateTeamMutation();

  // Initialize form data when team data loads
  useEffect(() => {
    if (teamData?.team) {
      setTeamName(teamData.team.name || "");
      
      // Map facilitators from team data
      if (teamData.team.facilitators) {
        setFacilitators(teamData.team.facilitators.map((f: any) => ({
          id: f.id,
      ...
      // Map learners from team data
      if (teamData.team.learners) {
        setLearners(teamData.team.learners.map((l: any) => ({
          id: l.id,
      ...
      // Map courses from team data
      if (teamData.team.courses) {
        setCourses(teamData.team.courses.map((c: any) => ({
          id: c.id,
          name: c.name || c.title,
                ...
                <div className="space-y-2 max-h-[300px] overflow-y-auto">
                  {facilitators.length > 0 ? (
                    ...
                    <div className="text-center py-8 text-gray-500">
                      No facilitators assigned to this team.
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2 max-h-[300px] overflow-y-auto">
                  {learners.length > 0 ? (
                    ...
                    <div className="text-center py-8 text-gray-500">
                      No learners assigned to this team.
                    </div>
                  )}
                </div>
              </div>
